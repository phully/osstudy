...
void kSetCursor(int iX,int iY)
{
    ...
    iLinerValue = iY * CONSOLE_WIDTH + iX;

    kOutPortByte(VGA_PORT_INDEX,VGA_INDEX_UPPERCURSOR);

    kOutPortByte(VGA_PORT_DATA,iLinerValue >> 8);

    kOutPortByte(VGA_PORT_INDEX,VGA_INDEX_LOWERCURSOR);

    kOutPortByte(VGA_PORT_DATA,iLinerValue & 0xFF);

    gs_stConsoleManager.iCurrentPrintOffset = iLinerValue;
}
