...
BOOL kFindMPFloatingPointerAddress(QWORD *pstAddress)
{
    ...
    for(pcMPFloatingPointer=(char *)qwEBDAAddress;pcMPFloatingPointer <= ((char *)qwEBDAAddress + 1024);pcMPFloatingPointer++)
    {
    ...
    for(pcMPFloatingPointer=((char *)qwSystemBaseMemory - 1024);pcMPFloatingPointer <= (char *)qwSystemBaseMemory;pcMPFloatingPointer++)
    {
        if(kMemCmp(pcMPFloatingPointer,"_MP_",4) == 0)
        {
...
void kPrintMPConfigurationTable(void)
{
    ...
    kPrintf("Feature Byte 2 : 0x%X ",pstMPFloatingPointer[1]);
    if(pstMPFloatingPointer->vbMPFeatureByte[1] & MP_FLOATINGPOINTER_FEATUREBYTE2_PICMODE)
    {
    ...
    kPrintf("==================== MP Configuration Header ===================\n"); 
